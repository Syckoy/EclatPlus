...
- Vérification de mise à jour **uniquement au lancement** (pas en boucle)
...
## Mises à jour
Au démarrage, l’appli consulte ce dépôt :
1. la dernière [release GitHub](https://github.com/Syckoy/EclatPlus/releases)
2. sinon `version.json` sur `main`
S’il existe une version plus récente, une invitation s’affiche. Rien n’est téléchargé ni écrit dans tes réglages.
Pour publier une version :
1. Augmenter `Version` dans `EclatPlus.csproj` et `"version"` dans `version.json`
2. Pousser sur `main`
3. Créer une release taguée `vX.Y.Z`
## Licence
Apache License 2.0
